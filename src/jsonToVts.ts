...
function writeVtsFile(node: VTNode) {
	...
	for (const [key, value] of Object.entries(node.values)) {
		...
		else if (value && typeof value === "object") result = `(${formatNum(value.x)}, ${formatNum(value.y)}, ${formatNum(value.z)})`;

		if (result === true) result = "True";
		if (result === false) result = "False";
		if (result === undefined) result = "";
		if (typeof result == "number" && result.toString().includes("e")) result = formatNum(result);
...
function jsonToVts(node: VTNode) {
